...
class Bug:
    ...
    def load_output(self, output_dir):
        ...
        # load coverage
        if os.path.exists(self.path_to_cov_df) and os.path.exists(self.path_to_test_results):
            ...
            if self.num_test_cases != len(self.test_results):
                raise Exception(f"Expected # tests: {self.num_test_cases} != Actual # tests: {len(self.test_results)}")
        else:
            self.test_results = read_dfcpp_test_results(self.output_dir)     
            if self.num_test_cases != len(self.test_results):
                raise Exception(f"Expected # tests: {self.num_test_cases} != Actual # tests: {len(self.test_results)}")
            self.coverage_df = read_dfcpp_coverage(self.output_dir,
                only_covered=True, verbose=True, encoding="ISO-8859-1")
            # load test results  
            bug.save_cov_df()
        ...
        if self.failing_tests != self.failing_tests_GT:
            raise Exception(f"Expected failings: {self.failing_tests_GT} != actual failings: {self.failing_tests}")
    ...
    def save_cov_df(self):
        self.coverage_df.astype(
            pd.SparseDtype("int", 0)).to_pickle(self.path_to_cov_df)
...
def localize_function(bugs, errors, result_dir):
    ...
    for bug in bugs:
        try:
            ...
            print(f"(# lines, # tests, # failing tests): \
            ...
            if len(buggy_files) != len(buggy_file_path_map):
                for buggy_file in buggy_files:
                    if buggy_file in buggy_file_path_map.values():
                        continue
                    for file_path in bug.source_files:
                        if os.path.basename(file_path) == os.path.basename(buggy_file):
                            buggy_file_path_map[file_path] = buggy_file
                            break
            
            if len(buggy_files) != len(buggy_file_path_map):
                raise Exception(f"The coverage of buggy files {buggy_files - set(buggy_file_path_map.values())} is not measured.")

            failure_coverage = cov_df[failing_tests]
            is_in_failure_coverage = np.any(failure_coverage, axis=1)
            files_covered_by_failure= set(
                failure_coverage.index[is_in_failure_coverage].get_level_values("file"))

            print(f"# files covered by failure: {len(files_covered_by_failure)}")
            buggy_files_not_covered_by_failure = set(buggy_file_path_map) - files_covered_by_failure

            print(f"# buggy files not covered by failure: {len(buggy_files_not_covered_by_failure)}")
            if buggy_files_not_covered_by_failure:
                raise Exception(f"Buggy files {buggy_files_not_covered_by_failure} is not covered by failing test casess")

            buggy_components = set()
            buggy_functions = set()
            for component in bug.coverage_df.index:
                _filepath, _function, _line = component
                if _filepath in buggy_file_path_map and (buggy_file_path_map[_filepath], _line) in buggy_lines:
                    buggy_components.add(component)
                    if _function:
                        buggy_functions.add((_filepath, _function))
                    else:
                        raise Exception(f"The function information is not available in coverage data for buggy line: {component}")
            ...
            for formula in FORMULAE:
                ...
                for scheme in AGGR_SCHEMES:
                    func_scores = AGGR_SCHEMES[scheme](line_scores,
                        level=["file", "function"])
                    func_scores = func_scores.sort_values(by="score",
                        ascending=False)
                    func_scores["is_buggy_function"] = func_scores.index.isin(
                        buggy_functions)
                    for tb in TIE_BREAKERS:
                        func_scores[("rank", tb)] = (-func_scores["score"]).rank(
                            method=tb)
                    func_scores.to_pickle(
                        f"{result_dir}/{bug}_{formula}_function_{scheme}.pkl")
                
                    for buggy_function in buggy_functions:
                        row = [ bug.id, len(bug.source_files), len(bug.functions),
                            formula, scheme, buggy_function ]
                        row += [
            ...
            errors[bug.id] = str(e)
            continue
    columns = [ "bug", "num_total_files", "num_total_functions", "formula", 
        "aggregation_scheme", "buggy_function"] + [
    ...
    for bug in bugs:
        try:
            ...
            for formula in FORMULAE:
                ...
                for scheme in AGGR_SCHEMES:
                    file_scores = AGGR_SCHEMES[scheme](line_scores,level="file")
                    file_scores = file_scores.sort_values(by="score",
                        ascending=False)
                    file_scores["is_buggy_file"] = file_scores.index.isin(
                        buggy_files)
                    for tb in TIE_BREAKERS:
                        file_scores[("rank", tb)] = (
                            -file_scores["score"]).rank(method=tb)
                    file_scores.to_pickle(
                        f"{result_dir}/{bug}_{formula}_file_{scheme}.pkl")
                
                    for buggy_file in buggy_files:
                        row = [ bug.id, len(bug.source_files),
                            formula, scheme, buggy_file ]
                        row += [
            ...
            errors[bug.id] = str(e)
            continue
    columns = [ "bug", "num_total_files", "formula", 
        "aggregation_scheme", "buggy_file"] + [
            f"rank_{tb}" for tb in TIE_BREAKERS]
    return result_rows, columns, errors
...
def draw_plot(df, level):
    ...
    sdf = sdf.melt(id_vars=["formula", "aggregation_scheme"],
        value_vars=[f"acc_{n}" for n in N], var_name="measure")

    for formula, tdf in sdf.groupby("formula"):
        plt.figure(figsize=(12, 5))
        plt.title(f"{formula} (# total bugs = {df.bug.unique().shape[0]})")
        hue_order = sdf[(sdf.measure == f"acc_{N[0]}")
            & (sdf.formula == formula)].sort_values(by="value").aggregation_scheme
        ax = sns.barplot(data=tdf, x="measure", y="value", 
            hue="aggregation_scheme", hue_order=hue_order)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--level', '-l', type=str, default='function',
        help='function or file')
